angular.module('multichatApp').controller('messagesCtrl', function ($scope, webSocketManager, $uibModal) {
    ...
    $scope.showFileModal = function (selectAudio, url, accept, cancel) {
        var modalInstance = $uibModal.open({
            ...
            if (result === "Too big")
            {
                webSocketManager.messagesManagement.showError("The file cannot be sent because it exceeds the maximum allowed size");
            }
            else
            {
                switch (result.type) {
